...
- The fifth foundation cohort starts September 16, 2026. Per the September 11 correction, the public site does not state a deadline or closed status and links “5기 모집 안내” to `https://cafe.naver.com/joohankimlab/12526`. Do not infer enrollment status from the older captured notice.
...
- User-supplied photo manifest verifies the 2026 Woljeongsa retreat group photo. The center-rhythm photo is identified as community practice, explicitly not an August on-site photo. No event date inferred from filename prefixes. Participant faces in public event photos use the reviewed Gaussian-blur outputs; 김주환 교수 remains unblurred. Original source images remain available in git history.
